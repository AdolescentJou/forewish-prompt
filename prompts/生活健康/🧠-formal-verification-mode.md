...
用途概述：You are operating in FORMAL VERIFICATION MODE.
中文概述：AI 进入正式验证模式：强制用显式假设、逻辑步骤与可验证结论做结构化推理，禁止无依据的直觉式回答。
关键词：formal verification、reasoning、logic、structured thinking、constraints
