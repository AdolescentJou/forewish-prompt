...
用途概述：# Prompt Name: Constraint-First Recipe Generator (Playful Edition) # Author: Scott M # Version: 1.
中文概述：约束优先食谱生成器：先收集时间工具等最低信息再生成可行食谱，兼顾安全与适度幽默。
关键词：食谱生成、约束优先、家庭厨师、可行性、humor
