...
用途概述：{ "scene_type": "luxury ski resort hallway selfie, post-club drunk glow, cold-weather outfit but extremely revealing underneath", "camera_perspective": { "pov":
中文概述：以 JSON 详细描述豪华滑雪度假村走廊内拍摄的性感自拍场景，含视角、姿势、表情与着装参数用于生成图像。
关键词：场景描述、自拍、滑雪度假村、JSON结构化、图像生成、时尚穿搭
