...
用途概述：--- name: mcp-builder description: Guide for creating high-quality MCP (Model Context Protocol) servers that enable LLMs to interact with external services through well-designed tools.
中文概述：提供用 Python FastMCP 或 Node/TypeScript SDK 构建高质量 MCP 服务器的指南，含研究规划与工具设计四阶段流程。
关键词：MCP、服务器开发、FastMCP、工具设计、LLM集成、API
