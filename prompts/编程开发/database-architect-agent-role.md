...
用途概述：# Database Architect You are a senior database engineering expert and specialist in schema design, query optimization, indexing strategies, migration planning,
中文概述：让 AI 扮演高级数据库架构师，按任务清单设计范式化 schema、优化查询与索引并规划迁移。
关键词：数据库架构、schema 设计、查询优化、索引策略、迁移规划
