...
用途概述：--- name: xcode-mcp description: Guidelines for efficient Xcode MCP tool usage.
中文概述：提供 Xcode MCP 工具使用准则 skill，按 token 成本对照表说明何时用 Xcode MCP（构建测试等）何时用标准工具。
关键词：Xcode、MCP、工具指南、iOS、token 成本、Skill
