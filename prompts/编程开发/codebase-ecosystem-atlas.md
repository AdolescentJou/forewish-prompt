...
用途概述：--- name: codebase-ecosystem-atlas description: Run a read-only, static-first analysis across a multi-repository software ecosystem and generate architecture ma
中文概述：让 AI 对多仓库软件生态做只读静态分析，生成架构地图、服务目录、业务流与安全 CI/CD 洞察等活文档。
关键词：多仓库分析、架构地图、服务目录、活文档、CI/CD、代码指标
