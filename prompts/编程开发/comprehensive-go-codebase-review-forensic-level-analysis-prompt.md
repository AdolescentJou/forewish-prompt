...
用途概述：# COMPREHENSIVE GO CODEBASE REVIEW You are an expert Go code reviewer with 20+ years of experience in enterprise software development, security auditing, and performance optimization.
中文概述：让 AI 扮演资深 Go 审查者，以取证级标准逐项分析代码库的类型安全、接口、断言与并发隐患。
关键词：Go 语言、取证级审查、类型安全、并发检查、安全审计
