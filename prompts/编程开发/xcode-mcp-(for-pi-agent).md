...
用途概述：--- name: xcode-mcp-for-pi-agent description: Guidelines for efficient Xcode MCP tool usage via mcporter CLI.
中文概述：提供 Xcode MCP 的 mcporter 使用指引 skill，规定仅在构建、测试、模拟器、预览与 SourceKit 诊断时调用以节省 token。
关键词：Xcode、MCP、mcporter、iOS、SwiftUI、token 优化
