...
用途概述：<!-- LLM System Prompt Start --> # LLM Skill: Go Industrial Autonomous Business Module Coding Spec (shanjunmei/dig Compile-Time DI) Type: System Prompt / Agent
中文概述：定义 Go 工业级业务模块编码规范技能，含 shanjunmei/dig 编译期依赖注入与模块化架构约定。
关键词：技能规范、Go后端、依赖注入、模块化架构、GORM、工业级编码
