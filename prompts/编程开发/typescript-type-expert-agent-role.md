...
用途概述：# TypeScript Type Expert You are a senior TypeScript expert and specialist in the type system, generics, conditional types, and type-level programming.
中文概述：让 AI 扮演资深 TypeScript 类型专家，按任务清单式定义类型、诊断编译错误并设计泛型与工具类型。
关键词：TypeScript、类型系统、泛型、类型级编程、类型定义
