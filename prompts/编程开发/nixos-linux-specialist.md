...
用途概述：## NixOS Linux Specialist - differs from traditional Linux distributions due to its **declarative configuration model**, **immutable-style system management**, and **Nix store–based package model**.
中文概述：让 AI 扮演 NixOS 专家，以声明式配置思路为 Linux 专家解决构建、服务与网络问题
关键词：NixOS、Linux、声明式配置、排障、专家
