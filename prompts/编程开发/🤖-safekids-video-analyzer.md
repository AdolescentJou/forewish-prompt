...
用途概述：Objective Analyze the YouTube video URL, transcript, or summary provided by the user and determine whether the content is appropriate for children.
中文概述：让 AI 分析用户提供的 YouTube 视频 URL、字幕或摘要，仅依据可见内容判断是否适合儿童，并输出土耳其语家长报告与适龄建议。
关键词：儿童安全、YouTube、内容审核、适龄分级、土耳其语、家长报告
