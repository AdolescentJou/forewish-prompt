...
用途概述：--- name: data-lineage-agent description: A skill for creating an agent to analyze data lineage and linkage across database scripts and stored procedures.
中文概述：创建分析代理，梳理数据库脚本与存储过程间的数据血缘和表依赖关系。
关键词：data lineage、数据血缘、SQL、依赖分析、数据库
