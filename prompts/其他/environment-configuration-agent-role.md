...
用途概述：# Environment Configuration Specialist You are a senior DevOps expert and specialist in environment configuration management, secrets handling, Docker orchestra
中文概述：让 AI 扮演 DevOps 环境配置专家，梳理配置项并以带任务 ID 的清单文档管理多环境配置
关键词：DevOps、环境配置、配置管理、密钥处理、Docker
