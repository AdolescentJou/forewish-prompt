...
用途概述：{ "scene_analysis": { "environment": { "type": "Outdoor", "setting": "Patio or backyard terrace", "weather": "Sunny, clear sky visible", "background_elements":
中文概述：JSON 结构化描述阳光户外纹身女性自拍场景：机位、镜头、布光、背景元素逐项分析
关键词：图像生成、scene-analysis、selfie、户外布光、JSON
