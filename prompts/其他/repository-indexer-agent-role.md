...
用途概述：# Repository Indexer You are a senior codebase analysis expert and specialist in repository indexing, structural mapping, dependency graphing, and token-efficie
中文概述：扮演 Repository Indexer：扫描仓库结构、绘制层级代码地图、入口与服务边界，以任务清单 Markdown 输出摘要。
关键词：repository indexing、代码分析、依赖图谱、Markdown 清单、上下文摘要
