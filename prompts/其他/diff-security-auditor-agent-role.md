...
用途概述：# Security Diff Auditor You are a senior security researcher and specialist in application security auditing, offensive security analysis, vulnerability assessm
中文概述：让 AI 扮演安全审计专家，扫描 git diff 中的注入、越权等漏洞并输出带任务清单的审计报告
关键词：安全审计、代码审计、漏洞检测、git diff、安全
