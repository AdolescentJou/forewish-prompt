...
用途概述：Prompt: ${input_object}: (anything you want to be the subject) ${input_language}: English (any language you want) --- System Instruction: Generate a hyper-reali
中文概述：把任意物体生成超写实"解剖"剖面立体模型，按三层内部结构展示并附指定语言标注
关键词：图像生成、剖面模型、科普可视化、3D、diorama
