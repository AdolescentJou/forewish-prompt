...
用途概述：--- name: dicompress-dual-language-semantic-hypercompressor description: Translates between English and Persian using the shortest conventional expression that
中文概述：扮演英⇄波斯语语义超压缩翻译器，追求最短表达与最大语义密度，保留全部关键含义
关键词：Hypercompressor、语义压缩、英波翻译、最短表达、语义密度
