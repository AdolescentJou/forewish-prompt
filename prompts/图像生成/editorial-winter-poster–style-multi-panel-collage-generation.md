...
用途概述：{ "meta_protocols": { "reference_adherence": { "instruction": "Use the provided male face photo as a strict reference_image.
中文概述：以男性面部照片为严格参考（零偏差），生成冬日着装、candid iPhone 质感的编辑风多面板拼贴海报。
关键词：拼贴海报、reference image、冬日穿搭、多面板、编辑风
