...
用途概述：{ "meta": { "description": "Structured prompt for generating an isometric city diorama in a miniature 3D style, with weather and environment adaptive to the specified city.
中文概述：AI 按 JSON 结构生成指定城市（如 San Francisco）的 isometric 微型 3D diorama，建筑材质与天气随城市自适应。
关键词：isometric、city-diorama、micro-diorama、3D、自适应天气
